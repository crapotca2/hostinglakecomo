...
const COPY = {
  ...
  ru: {
    docTitle: "Юридические уведомления",
    description:
      "Идентификационная информация об операторе сайта Хост Комо в соответствии с Законодательным декретом Италии 70/2003 об электронной торговле.",
    ...
    intro: (
      <>
        Настоящее юридическое уведомление публикуется в соответствии со
        статьями 7 и 8 Законодательного декрета Италии 70/2003 об
        электронной торговле и содержит идентификационную информацию об
        операторе сайта <strong>hostcomo.com</strong>.
      </>
    ),
    sections: [
      {
        h: "1. Идентификация оператора",
        body: (
          <>
            Сайтом hostcomo.com управляет{" "}
            <span className="text-foreground font-medium">Angelo Talarico</span>
            , физическое лицо, проживающее в Комо (Италия), действующее от
            своего имени в сфере co-hosting и управления краткосрочной
            арендой на озере Комо. Поскольку деятельность ведётся
            физическим лицом, а не юридическим лицом, отсутствуют название
            компании, номер НДС, код REA, юридический адрес и сертифицированная
            электронная почта (PEC), которые необходимо было бы раскрывать.
            По всем вопросам коммуникации обращайтесь к контактам,
            указанным в следующем разделе.
          </>
        ),
      },
      {
        h: "2. Контактные данные",
        body: (
          <>
            Для любых формальных или неформальных сообщений, касающихся
            Сайта и Услуг, можно использовать следующие каналы: email{" "}
            <a
              href="mailto:info@hostcomo.com"
              className="text-primary hover:underline"
            >
            ...
            , телефон <strong>+39 351 718 0435</strong>, операционный
            адрес <strong>Комо, Италия</strong>.
          </>
        ),
      },
      {
        h: "3. Хостинг и инфраструктура",
        body: (
          <>
            Сайт размещён на облачной инфраструктуре Vercel Inc., 440 N
            Barranca Avenue #4133, Covina, CA 91723, Соединённые Штаты
            Америки, с основными датацентрами в регионе ЕС Франкфурт
            (eu-west). Данные передаются по HTTPS с TLS-сертификатом,
            автоматически выдаваемым Let&apos;s Encrypt. Аутентификация
            пользователей клиентского раздела осуществляется через OAuth
            Google LLC.
          </>
        ),
      },
      {
        h: "4. Авторские права и товарные знаки",
        body: (
          <>
            Товарные знаки «Host Como» и «Como Host», логотип, wordmark
            «como» и всё графическое и текстовое содержимое, опубликованное
            на Сайте, защищены итальянским и европейским законодательством
            об авторском праве и промышленной собственности. Фотографии
            управляемых объектов опубликованы с разрешения соответствующих
            собственников, которые сохраняют право собственности на снимки.
            Любое содержимое третьих сторон (логотипы Airbnb, Booking.com,
            Expedia, Google и других партнёров) используется исключительно
            в идентификационных целях и остаётся собственностью
            соответствующих правообладателей.
          </>
        ),
      },
      {
        h: "5. Характер услуг и ограничение ответственности",
        body: (
          <>
            <p>
              Сайт имеет информационную и витринную функцию для услуг
              co-hosting, предлагаемых на озере Комо. Он не является
              платформой посредничества в сфере недвижимости и не является
              платформой прямого бронирования: бронирование проживания в
              управляемых объектах осуществляется на официальных каналах
              Airbnb, Booking.com и Expedia через объявления отдельных
              собственников, регулируемые условиями этих платформ.
            </p>
            <p className="mt-4">
              Идентификационные коды (CIN, CIR и аналогичные), требуемые
              итальянским законодательством для туристических объектов,
              являются ответственностью отдельных собственников и
              отображаются в их объявлениях и контрактах. Сайт не несёт
              ответственности за нормативное соответствие отдельных
              управляемых объектов, которое остаётся на держателе каждого
              объекта.
            </p>
          </>
        ),
      ...
      {
        h: "6. Связанные документы",
        body: (
          <>
            <p>
              Настоящее юридическое уведомление дополняет следующие
              документы, доступные на Сайте:
            </p>
            <ul className="space-y-2 mt-3 list-disc pl-6">
              <li>
                <a href="/privacy" className="text-primary hover:underline">
                ...
                — GDPR-уведомление об обработке персональных данных
              </li>
              <li>
                <a href="/cookies" className="text-primary hover:underline">
                  Политика cookies
                </a>{" "}
                — уведомление о cookies и технологиях отслеживания
              </li>
              <li>
                <a href="/terms" className="text-primary hover:underline">
                  Условия использования
                </a>{" "}
                — условия использования Сайта и калькуляторов
              </li>
            </ul>
          </>
        ),
